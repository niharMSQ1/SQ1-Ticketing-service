...
def start_scheduler():
    ...
    start_time = datetime.now(pytz.UTC).replace(hour=9, minute=20, second=0, microsecond=0)
   ...
   # scheduler.add_job(freshservice_call_create_ticket, CronTrigger(hour=start_time.hour, minute=start_time.minute, day_of_week='*', start_date=start_time))
    ...
    scheduler.add_job(updateExploitsAndPatchesForFreshservice, CronTrigger(hour=start_time.hour, minute=(start_time.minute + 7) % 60, day_of_week='*', start_date=start_time))
