...
def start_scheduler():
    ...
    start_time = datetime.now(pytz.UTC).replace(hour=6, minute=30, second=0, microsecond=0)
    ...
    scheduler.add_job(freshservice_call_create_ticket, CronTrigger(hour=start_time.hour, minute=(start_time.minute + 3) % 60, day_of_week='*', start_date=start_time))
