...
def start_scheduler():
    ...
    start_time = datetime.now(pytz.UTC).replace(hour=12, minute=26, second=0, microsecond=0)
    ...
    scheduler.add_job(updateExploitsAndPatchesForJira, CronTrigger(hour=start_time.hour, minute=(start_time.minute + 11) % 60, day_of_week='*', start_date=start_time))
    scheduler.add_job(updateExploitsAndPatchesForTrello, CronTrigger(hour=start_time.hour, minute=(start_time.minute + 13) % 60, day_of_week='*', start_date=start_time))

    scheduler.add_job(changeVulnerabilityStatusForFreshService, CronTrigger(hour=start_time.hour, minute=(start_time.minute + 15) % 60, day_of_week='*', start_date=start_time))
    scheduler.add_job(changeVulnerabilityStatusForJira, CronTrigger(hour=start_time.hour, minute=(start_time.minute + 33) % 17, day_of_week='*', start_date=start_time))
    scheduler.add_job(changeVulnerabilityStatusForTrello, CronTrigger(hour=start_time.hour, minute=(start_time.minute + 19) % 60, day_of_week='*', start_date=start_time))
